...
namespace ADM.Store.Service.Services.Sales
{
    internal class CustomerService : ICustomerService
    {
        ...
        public async Task<CustomerDetailsModel?> GetAsync(string firstName, string lastName)
        {
            var customerRegistered = await _customerRepository.ExistsAsync(firstName, lastName).ConfigureAwait(false);
            if (customerRegistered != null) MapModelToLocal(customerRegistered);
            return customerRegistered;
        }
        ...
        {
            var idCustomerCreated = await _customerRepository.CreateAsync(customerCreate.FirtName, customerCreate.LastName, customerCreate.PhoneNumber, customerCreate.Email).ConfigureAwait(false);
        ...
        public async Task UpdateAsync(CustomerUpdateModel customerUpdate)
        {
            ...
            if(customerRegistered.FirtName != customerUpdate.FirtName || customerRegistered.LastName != customerUpdate.LastName || customerRegistered.PhoneNumber != customerUpdate.PhoneNumber
